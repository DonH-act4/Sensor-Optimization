...
def run_dataset(args: argparse.Namespace, dataset: str, device: torch.device) -> dict:
    ...
    if resume_path is not None:
        # Project checkpoints contain optimizer/scheduler state and numpy
        # normalization arrays, so PyTorch 2.6+'s default weights_only=True
        # loader rejects them. These checkpoints are generated by this training
        # script; do not use --resume-checkpoint with untrusted files.
